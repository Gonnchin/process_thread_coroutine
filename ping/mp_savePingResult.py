...
#     for i in result:
#         result_list.append(i.get())
#     return result_list


def process_start(src_ip, name):
    """协程"""
    result = []
    # result_tmp格式：[<Greenlet at 0x217f910>]
    result_tmp = gevent.joinall([gevent.spawn(getDelayTime, src_ip, name)])
    for i in result_tmp:
        result = i.get()
    # result格式：{'1.58.1.52': ['8', '2018-01-19 14:53:59:05']}
    return result
...
def getPingResult(src_ip, count):
    ...
    result_tmp = [pool.apply_async(process_start, args=(src_ip, test_name[0])) for test_name in test_name_list]
...
def savePingResult(src_ip, count):
    ...
    # 获取测试数据
    test_result = getPingResult(src_ip, count)
    #print "test_result: %s" % test_result
    # 连接influxdb，保存数据
    client = InfluxDBClient(host='XXXX', port=8086, database='pingResult')
